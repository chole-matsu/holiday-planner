...
export function decorateControls(){for(const [id,name,label] of [['today','clock','今'],['view-schedule','calendar','予定'],['view-categories','grid','カテゴリ'],['view-packing','bag','持ち物'],['add-event','plus','予定追加'],['undo','undo','元に戻す']]){const button=document.getElementById(id);button.replaceChildren(icon(name),document.createTextNode(label));}document.getElementById('today').setAttribute('aria-label','現在の日時にジャンプ');}
